...
- **MQTT parse hot path**: `PskMessageHandler` scans the raw `ReadOnlySequence<byte>` payload with `Utf8JsonReader` (no `JsonDocument` DOM, no `ConvertPayloadToString`), reading only `b`/`rc`/`rl`/`sc`/`sl`. ~1.1M msg/s vs ~196k msg/s for the old string+DOM path. Do NOT revert to `JsonDocument.Parse`/string payloads on the receive path.
...
- **`MqttSettings.Subscriptions`** is a list of connections, each with its own topic. Default (empty list) = single all-bands sub `pskr/filter/v2/+/+/+/+/+/+/+`. Each sub inherits `Host`/`Port` unless overridden (`MqttSettings.GetSubscriptions()` resolves this). Per-band topics use the `#` wildcard, e.g. `pskr/filter/v2/20m/#`. One `PskReporterWorker` is registered per subscription; all share the single `Channel<Station>`. Message fields: `rc`/`rl` (receiver), `sc`/`sl` (sender), `b` (band).
- **MQTT client IDs must be unique per connection** — the broker kicks an existing client whose ID collides (session takeover). Each worker uses `gridifier-{instanceId}-{subscriptionIndex}` (`instanceId` = random per process, stable across reconnects). Do NOT set a bare constant client ID or the N workers sharing the broker will disconnect each other.
...
- **MQTT transport is pluggable behind `IMqttConnection`** (`MqttConnectionFactory`, `Mqtt:Transport` config): `"pulse"` (default) uses `PulseMqttConnection` (RawMqttClient), `"mqttnet"` uses `MqttnetConnection`. `PskReporterWorker` is transport-agnostic — it wires `PskMessageHandler` + stats to `IMqttConnection` events. Pulse allocates ~2x less per message on the receive path than MQTTnet but costs the SAME CPU (the J1900 ceiling is genuine work — ~620µs/msg of JSON+receive — not GC). Add a new client by implementing `IMqttConnection` + a factory case, not by touching the worker.
- **AOT/NativeAOT is NOT supported and will not help**: `StatsController` serializes an anonymous type via reflection-based JSON (default MVC `AddControllers()`); trimming/AOT requires reinventing source-generated `JsonSerializerContext` (deliberately removed as dead code) and switching the Docker base image to a platform-specific runtime. It also would NOT reduce the steady-state J1900 CPU (throughput-bound, not JIT/startup-bound). Do NOT attempt trimming/AOT without first converting stats serialization to source-gen.
